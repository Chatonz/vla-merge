...
def _resolve_suffix(
    modules: dict[str, torch.nn.Module], suffix: str
...
class BlockCalibrationCollector:
    ...
    def __init__(self, policy: torch.nn.Module) -> None:
        ...
        self.request_mode = (
            os.environ.get("PI05_BLOCK_REGMEANPP_REQUEST_MODE", "call_reservoir").strip().lower()
        )
        if self.max_calls <= 0:
        ...
        self.structured_episode_slots_by_prompt: dict[int, dict[int, dict[int, list[int]]]] = {}
    ...
    def capture_task_texts(self, tasks: Any) -> None:
        ...
        if len(task_list) != 1:
            raise ValueError(
                f"Task-stratified calibration currently requires eval.batch_size=1; received {len(task_list)} task descriptions"
            )
    ...
    def _capture_prompt_tokens_from_policy_batch(self, batch: dict[str, Any]) -> None:
        ...
        if tokens.ndim != 2 or attention_mask.shape != tokens.shape:
            raise ValueError(
                f"Expected task tokens/mask with the same [batch, sequence] shape, got {tuple(tokens.shape)} and {tuple(attention_mask.shape)}"
            )
        if tokens.shape[0] != 1:
            raise ValueError(
                f"Task-stratified calibration currently requires eval.batch_size=1; received batch size {tokens.shape[0]}"
            )
        valid_token_ids = (
    ...
    def _begin_structured_request(self) -> None:
        ...
        if self.request_mode == "initial":
            reservoir_index = request_index if request_index < self.requests_per_episode else None
            ...
            salt = zlib.crc32(self.task.encode("utf-8")) ^ signature ^ self.current_episode_serial
            draw = (request_index + 1) * 1103515245 + salt & 2147483647
            candidate = draw % (request_index + 1)
            reservoir_index = candidate if candidate < self.requests_per_episode else None
    ...
    def _end_structured_request(self) -> None:
        ...
        if missing:
            raise RuntimeError(
                f"Request {self.current_request_index} did not reach flow indices {sorted(missing)}; observed {self.current_flow_index} Action Expert calls"
            )

    def _reservoir_slot(
        self, prompt_signature: int | None
    ) -> tuple[int | None, list[dict[str, torch.Tensor]]]:
        self.seen_calls += 1
        if self.max_calls_per_prompt > 0:
            ...
            if self.episode_aware:
                ...
                selected = (episode_seen * 1103515245 + salt & 2147483647) % episode_seen == 0
            ...
            slot = (seen * 1103515245 + salt & 2147483647) % seen
        ...
        slot = (self.seen_calls * 1103515245 + salt & 2147483647) % self.seen_calls
    ...
    def _layer_hook(
        ...
        if self.flow_indices:
            ...
            if not self.current_request_selected or structured_flow_index not in self.flow_indices:
                return
        ...
        if self.full_prefix:
            if self.latest_prefix is None:
                raise RuntimeError(
                    "Action suffix started without a captured vision/language prefix"
                )
    ...
    def install(self) -> None:
        if self.max_calls_per_prompt > 0:
            ...
            if self.episode_aware:
                ...
                def reset_with_episode_tracking(
                    _policy: torch.nn.Module, *args: Any, **kwargs: Any
                ) -> Any:
                    self._start_episode()
                    return self.original_policy_reset(*args, **kwargs)

                self.policy.reset = types.MethodType(reset_with_episode_tracking, self.policy)

            def predict_action_chunk_with_prompt(
                _policy: torch.nn.Module, batch: dict[str, Any], *args: Any, **kwargs: Any
            ) -> torch.Tensor:
            ...
            self.policy.predict_action_chunk = types.MethodType(
                predict_action_chunk_with_prompt, self.policy
            )
        self.handles.append(
            self.action_module.register_forward_pre_hook(self._input_hook("action"))
        )
        ...
        if self.full_prefix:
            ...
            self.handles.append(
                self.language_layer0.register_forward_pre_hook(
                    self._language_hook, with_kwargs=True
                )
            )
        self.handles.append(
            self.layer0.register_forward_pre_hook(self._layer_hook, with_kwargs=True)
        )
        print(
            f"Installed strict block calibration collector: task={self.task} max_calls={self.max_calls} max_calls_per_prompt={self.max_calls_per_prompt} requests_per_episode={self.requests_per_episode} episode_aware={self.episode_aware} full_prefix={self.full_prefix} request_mode={self.request_mode} flow_indices={self.flow_indices} layer={self.layer_name}",
            flush=True,
        )
    ...
    def save(self) -> None:
        ...
        if self.max_calls_per_prompt > 0:
            ...
            prompt_signatures = [signature for signature, group in ordered for _record in group]
            record_metadata = [
                metadata
        ...
        manifest = {
            ...
            "sampling": "structured_request_flow"
            if self.flow_indices
            else "prompt_stratified_reservoir"
            if self.max_calls_per_prompt > 0
            ...
            "samples_per_episode": self.requests_per_episode * len(self.flow_indices)
            if self.flow_indices
            else 1,
        ...
        print(
            f"Saved {len(records)} replay records across {len(self.records_by_prompt)} prompts after {self.seen_calls} denoise calls to {self.tensor_output}",
            flush=True,
        )
